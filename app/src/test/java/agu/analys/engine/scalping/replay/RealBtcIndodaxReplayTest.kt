...
class RealBtcIndodaxReplayTest {
    ...
    fun `real BTCIDR replay produces causal checkpoint audit`() = runBlocking {
        ...
        val reportText = buildString {
            ...
            appendLine("- BUY signals: ${report.totalSignalsTriggered}")
            ...
            appendLine()
            appendLine("| Checkpoint | Rejections | Distribution | Missed after checkpoint |")
            appendLine("|---|---:|---:|---:|")
            appendLine("| Step 1 | ${report.bottleneck.step1Rejections} | ${"%.2f".format(report.bottleneck.step1RejectionPct)}% | ${report.bottleneck.missedAfterStep1} |")
            appendLine("| Step 2 | ${report.bottleneck.step2Rejections} | ${"%.2f".format(report.bottleneck.step2RejectionPct)}% | ${report.bottleneck.missedAfterStep2} |")
            appendLine("| Step 3 | ${report.bottleneck.step3Rejections} | ${"%.2f".format(report.bottleneck.step3RejectionPct)}% | ${report.bottleneck.missedAfterStep3} |")
            appendLine("| Step 4 | ${report.bottleneck.step4Rejections} | ${"%.2f".format(report.bottleneck.step4RejectionPct)}% | ${report.bottleneck.missedAfterStep4} |")
            appendLine()
