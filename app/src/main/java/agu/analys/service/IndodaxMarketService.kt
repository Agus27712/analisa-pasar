...
object IndodaxMarketService {
    ...
    suspend fun fetchTicker(symbol: String, prevPrice: Double = 0.0): MarketTick? = withContext(Dispatchers.IO) {
        try {
            ...
            MarketTick(pair.uppercase().replace("_", ""), last, t.optString("high", "0").toDoubleOrNull() ?: last, t.optString("low", "0").toDoubleOrNull() ?: last, t.optString("vol_idr", "0").toDoubleOrNull() ?: 0.0, change ?: Double.NaN, System.currentTimeMillis())
        } catch (_: Exception) { null }
    }
    ...
    suspend fun fetchTickers(pairIds: List<String>): List<MarketTick> = withContext(Dispatchers.IO) {
        try {
            ...
            pairIds.mapNotNull { raw ->
                ...
                MarketTick(pair.uppercase().replace("_", ""), last, t.optString("high", "0").toDoubleOrNull() ?: last, t.optString("low", "0").toDoubleOrNull() ?: last, t.optString("vol_idr", "0").toDoubleOrNull() ?: 0.0, fetch24hChange(pair, last) ?: Double.NaN, System.currentTimeMillis())
            }
